...
作为一名开源爱好者，我曾为许多 Flutter 开源项目做过贡献，包括作为 [GetX](https://pub.dev/packages/get)（最受 Flutter 开发者喜爱的库） 的首位中文文档翻译者，以及为 [flutter_screenutil](https://pub.dev/packages/flutter_screenutil), [image](https://pub.dev/packages/image) 等常用库提交过 PR。

同时，我也发布维护了多个 package 回馈给社区，包括: [nested_scroll_view_plus](https://pub.dev/packages/nested_scroll_view_plus), [auto_hide_keyboard](https://pub.dev/packages/auto_hide_keyboard) 等。
...
这里我们选择了高德定位SDK，相较于系统原生的 GPS 定位服务，它采用 GPS + 北斗 + 基站（含5G基站）+ WIFI 的混合定位模式，可以根据设备所处环境，智能选择最佳的定位方式，在降低耗电的同时，保证定位的准确性。
...
这种布局通常由 header + body 两部分组成。其中头部高度一般不固定，用来展示用户头像和个人简介等信息，而 body 区域则是由不同标签分类组成的 Tabview，里面是用来展示用户作品和动态等信息的内容列表。
...
为了解决这个问题，我 Fork 了官方的 `NestedScrollView`，然后重写了内外 ScrollView 和 Coordinator 坐标之间的映射关系，并修复了一些已知问题。现在你可以在 [pub.dev](https://pub.dev/packages/nested_scroll_view_plus) 上直接搜索并使用修改后的 `NestedScrollViewPlus` 组件。
...
为此，我在高德地图插件的基础上，实现了使用 Flutter 原生 Widget 作为地图 Marker 的解决方案，为传统的地图交互体验，带来更多的想象空间。
